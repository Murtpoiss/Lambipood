...
function woocommerce_add_to_cart_message( $product_id ) {
	...
	if ( is_array( $product_id ) ) {
		...
		$added_text = sprintf( __( 'Lisatud &quot;%s&quot; Teie Ostukorvi.', 'woocommerce' ), join( __( '&quot; ja &quot;', 'woocommerce' ), array_filter( array_merge( array( join( '&quot;, &quot;', array_slice( $titles, 0, -1 ) ) ), array_slice( $titles, -1 ) ) ) ) );
	...
	if ( get_option( 'woocommerce_cart_redirect_after_add' ) == 'yes' ) :
		...
		$message 	= sprintf('<a href="%s" class="button">%s</a> %s', $return_to, __( 'Jätka Ostmist &rarr;', 'woocommerce' ), $added_text );

	else :

		$message 	= sprintf('<a href="%s" class="button">%s</a> %s', get_permalink( woocommerce_get_page_id( 'cart' ) ), __( 'Vaata Ostukorvi &rarr;', 'woocommerce' ), $added_text );
